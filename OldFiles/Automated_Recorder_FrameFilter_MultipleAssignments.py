...
def frameFilter(name,min_area,fps,height,width,save_path_test,fourcc_avi,fileName):
    ...
    prod11 = np.concatenate((prod1,no_movement))
    vid_length = 0
    for a in range(len(prod1)):
        if prod1[a] == 1:
            prod1_frame1 = cv2.cvtColor(test_frame_arr[a], cv2.COLOR_BGR2RGB)
            prod_out.write(prod1_frame1)
            vid_length += 1
        elif prod11[a+1] == 0 and prod1[a] == 1 and vid_length > 10:
            prod_out.release()
    ...
    # print("length frame array: "+str(len(test_frame_arr)))
    # print("length prod1: " + str(len(prod1)))
...
assignments = ["FESTO104", "FESTO112", "FESTO121"]
path2_dh_DT = 'cif_plc_control'

filtered_video_per_assignment = 'C:\\Users\\20182615\\Documents\\Jaar 3\\BEP\\EncodingDecoding\\filtered_videos\\test2'
...
# running the code for all given assignments
...
#----------------------------------------------------------------------------------------------------------------------#
    ...
    inputs = [(names[0], min_areas[0], fps, heights[0], widths[0], save_path_filtered, fourcc_avi, vid_names[0]),
              (names[1], min_areas[1], fps, heights[1], widths[1], save_path_filtered, fourcc_avi, vid_names[1]),
              (names[2], min_areas[2], fps, heights[2], widths[2], save_path_filtered, fourcc_avi, vid_names[2]),
              (names[3], min_areas[3], fps, heights[3], widths[3], save_path_filtered, fourcc_avi, vid_names[3])]
...
grading_end = time.time()
grading_length = grading_end - grading_start

str_time = "The grading of assignments: "
str_time2 = " took: "
str_time3 = " seconds in total"

for assignment in assignments:
    str_time = str_time + ', ' + assignment
str_time = str_time + str_time2 + str(grading_length) + str_time3
print(str_time)
